...
Screen::Screen()
    ...
    , m_buffer2(new Uint32[SCREEN_WIDTH * SCREEN_WIDTH])
{
}
...
bool Screen::init()
{
  ...
  memset(m_buffer1.get(), 0x00, SCREEN_WIDTH * SCREEN_HEIGHT * sizeof(Uint32));
...
void Screen::setPixel(Uint32 x, Uint32 y, Uint8 red, Uint8 green, Uint8 blue)
{
  ...
  m_buffer1.get()[x + SCREEN_WIDTH * y] = color;
}

void Screen::update()
{
  SDL_UpdateTexture(m_texture, NULL, m_buffer1.get(), SCREEN_WIDTH * sizeof(Uint32));
  ...
  for (int row = -1; row <= 1; row++) {
    for (int col = -1; col <= 1; col++) {
      ...
      if (currentX >= 0 && currentY >= 0 && currentX < SCREEN_WIDTH && currentY < SCREEN_HEIGHT) {
        ...
        Uint32 currentColor = m_buffer2.get()[currentY * SCREEN_WIDTH + currentX];
